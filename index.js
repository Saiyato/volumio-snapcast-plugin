...
ControllerSnapCast.prototype.updateSpotifyImplementation = function()
{
	...
	{
		self.replaceStringInFile("--device ${outdev}", "--backend pipe --device " + self.config.get('spotify_pipe') + " --disable-audio-cache --initial-volume $\\{initvol\\}", "/data/plugins/music_service/volspotconnect2/volspotconnect2.tmpl");
		defer.resolve();
	}
	else if (imp == "spop")
	{
